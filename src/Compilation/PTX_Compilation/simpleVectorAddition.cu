//Source: NVIDIA Documentation: PTX Compiler Documentation
//Link: https://docs.nvidia.com/cuda/ptx-compiler-api/index.html#example-simple-vector-addition
...
int main(int _argc, char *_argv[])
{
    CUdevice dev;
    char name[100];
    CUDA_SAFE_CALL(cuInit(0)); // Must come first
    CUDA_SAFE_CALL(cuDeviceGet(&dev, 0));
    CUDA_SAFE_CALL(cuDeviceGetName(name, sizeof(name), dev));
    ...
    FILE *fp = fopen("output.cubin", "wb");
    if (fp) {
        fwrite(elf, 1, elfSize, fp);
        fclose(fp);
        printf("Saved CUBIN to output.cubin\n");
    } else {
        printf("Failed to save CUBIN to file\n");
    }
