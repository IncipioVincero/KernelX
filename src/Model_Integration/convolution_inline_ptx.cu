...
__global__ void convolution_ptx(float *I, const float *M, float *P, int C, int W, int H) {
...
void ptx_convolve(torch::Tensor input, torch::Tensor mask, torch::Tensor output) {
    int H = input.size(1);
    int W = input.size(2);
    int C = input.size(0);
    ...
    convolution_ptx<<<dimGrid, dimBlock>>>(
