    ...
    for (int k = 0; k < channels; ++k) {
        ...
        for (int i = 0; i < MASK_WIDTH; ++i) {
            for (int j = 0; j < MASK_WIDTH; ++j) {
                float val;
                asm volatile("ld.shared.f32 %0, [%1];" : "=f"(val) : "l"(&N_ds[ty + i][tx + j]));
                accum += val * M[i * MASK_WIDTH + j];
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    convolution_with_ptx<<<dimGrid, dimBlock>>>(deviceInputImageData, deviceMaskData, deviceOutputImageData, imageChannels, imageWidth, imageHeight);
